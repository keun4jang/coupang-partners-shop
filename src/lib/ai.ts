...
export function fallbackCopy(product: Product, displayNumber: number): VideoCopy {
  ...
  const presets: Record<string, { hook: string; empathy: string; b1: string; b2: string }> = {
    ...
    청소템: {
      ...
      empathy: "금방 신경 쓰이잖아요",
      b1: "하나쯤 있으면 편해 보여요",
      b2: "구석 청소할 때 은근 쓸 일 많고요",
    },
    수납템: {
      hook: "아이 둘 키우다 보면",
      empathy: "물건이 금방 늘어나잖아요",
      b1: "자잘한 물건 정리할 때 괜찮아 보여요",
    ...
    주방템: {
      ...
      b2: "하나 두면 생각보다 자주 쓸 것 같아요",
    },
    육아생활템: {
      hook: "아이 둘 키우는 집은",
      empathy: "이런 상황 정말 많죠",
      b1: "집에 두면 생각보다 자주 쓸 것 같아요",
      b2: "후기 많은 제품 중에 눈에 띄는 생활템이에요",
    },
  };

  const preset = presets[category] ?? {
    hook: pain ? `${pain}` : "살림하다 보면 이런 게",
    empathy: "은근 신경 쓰이잖아요",
...
- 권장 톤: "이런 거 하나 있으면 은근 편해요", "집에 두면 생각보다 자주 쓸 것 같아요", "아이 둘 키우다 보면 이런 상황 많죠", "~해 보여요", "~잖아요"
