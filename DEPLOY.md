...
3. Value 에 워커용 환경변수 전체를 `.env.local` 형식으로 붙여넣기 (필요 키:
   ...
   `UPLOAD_SCHEDULE`(업로드 슬롯 - DB(app_settings)가 우선이라 보통 넣지 않는다),
   (선택)`AI_API_KEY`)
...
> ✅ **우선순위**: `UPLOAD_SCHEDULE` 은 **app_settings(DB)가 환경변수보다 우선**이다
> (`design_template` 과 동일). 슬롯 수 = 하루 발행 편수라 자주 바꾸는 값이어서
> DB 하나로 관리한다. 워커 로그 첫머리에 `업로드 슬롯 출처=...` 로 어느 값을
> 썼는지 찍히므로, 안 바뀌는 것 같으면 그 줄을 먼저 본다.
...
> → 채널 계정으로 동의 → `exchange` → 나온 값을 `YOUTUBE_OAUTH_CLIENT_ID`/
> `YOUTUBE_OAUTH_CLIENT_SECRET`/`YOUTUBE_OAUTH_REFRESH_TOKEN` 에 등록. 해당 구글 클라우드
> 프로젝트에서 **YouTube Data API v3** 도 활성화해야 한다. 미설정 시 유튜브만 조용히 건너뛴다.
>
> 📸 **인스타 릴스 자동 게시**(Instagram 로그인 방식, `graph.instagram.com`): 앱 심사·페이스북
> 페이지 연결 불필요 - ① 인스타를 프로페셔널 계정으로 전환 ② 메타 앱 "역할"에서 그 계정을
> **Instagram 테스터**로 추가 → 초대 수락 ③ `instagram_business_basic` +
> `instagram_business_content_publish` 권한으로 액세스 토큰 생성 ④ `GET graph.instagram.com/me`
> 로 얻은 id 와 함께 `INSTAGRAM_ACCESS_TOKEN`/`INSTAGRAM_BUSINESS_ACCOUNT_ID` 에 등록.
> 토큰은 장기 토큰(약 60일)이라 만료 전 갱신 필요. 자세한 절차는 `src/lib/instagram.ts`
> 상단 주석 참고. 미설정 시 인스타 업로드만 조용히 건너뛴다.
